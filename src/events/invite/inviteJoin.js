        ...
        if (messageData && messageData.inviteJoin) {
            ...
            welcomeSchema.findOne({ Guild: member.guild.id }, async (err, channelData) => {
                if (channelData) {
                    ...
                    client.embed({
                        title: `👋・Selamat datang`,
                        desc: `Lah gimana caranya **${member} | ${member.user.tag}** bisa join`
                    }, channel).catch(() => { })
                }
            })
        }
            ...
            if (messageData) {
                ...
                welcomeSchema.findOne({ Guild: member.guild.id }, async (err, channelData) => {
                    if (channelData) {
                        ...
                        client.embed({
                            title: `👋・Selamat datang`,
                            desc: `**${member} | ${member.user.tag}** diundang oleh ${inviter.tag} **(${data.Invites} invites)**`
                        }, channel)
                    }
                })
            }
            ...
            if (messageData) {
                ...
                welcomeSchema.findOne({ Guild: member.guild.id }, async (err, channelData) => {
                    if (channelData) {
                        ...
                        await client.embed({
                            title: `👋・Selamat datang`,
                            desc: `**${member} | ${member.user.tag}** diundang oleh ${inviter.tag} **(1 invites)**`
                        }, channel).catch(() => { })
                    }
                })
            }
