        ...
        banSchema.findOne({ User: interaction.user.id }, async (err, data) => {
            if (data) {
                return client.errNormal({
                    error: "Kamu telah di banned oleh developer BOT ini",
                    type: 'ephemeral'
                }, interaction);
            }
            else {
                ...
                if (!cmd){
                    ...
                    if (cmdx) {
                        ...
                        if (cmdx.Action == "Normal") {
                            ...
                            interaction.editReply({ content: "Sudah mengirim pesan melalui DM" });
                            return interaction.user.send({ content: cmdx.Responce }).catch((e) => {
                                client.errNormal(
                                    {
                                        error: "Tidak bisa DM, silahkan nyalakan DM terlebih dahulu!",
                                        type: 'ephemeral'
                                    },
                                    interaction,
                                );
                ...
                interaction.reply({ files: [image], fetchReply: true }).then(function (msg) {
                    ...
                    interaction.channel.awaitMessages({ filter, max: 1 }).then(response => {
                        if (response.first().content === captcha.value) {
                            ...
                            client.succNormal({
                                text: "Kamu sudah terverifikasi! Selamat datang di Infinesting"
                            ...
                            client.errNormal({
                                error: "Kamu tidak menjawab captcha dengan benar!",
                                type: 'editreply'
                ...
                async (err, data) => {
                    ...
                    for (let i = 0; i < interaction.values.length; i++) {
                        ...
                        if ((i + 1) === interaction.values.length) {
                            interaction.reply({
                                content: `Role sudah diperbaharui: ${roles}`,
                                ephemeral: true,
                            });
                        }
                    }
